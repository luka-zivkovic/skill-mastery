...
6. Stop at the spec. Hand the confirmed spec to simplest-thing only after the user confirms or answers — never silently proceed.
...
`## Need from you` (numbered blocking questions, ≤5) → one line:
`Ready to build on confirm — hand the confirmed spec to simplest-thing.`
